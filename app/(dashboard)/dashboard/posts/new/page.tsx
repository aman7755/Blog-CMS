...
export default function NewPostPage() {
  ...
  const turndownService = new TurndownService({
    rules: {
      cardBlock: {
        filter: (node) =>
          node.nodeName === 'DIV' && node.getAttribute('data-type') === 'card-block',
  ...
  const insertCardBlock = () => {
    ...
    if (cardId && editor.current) {
      editor.current.commands.insertCardBlock({ cardId, position: 0 }); // Position will be updated on save
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      cardBlocks.forEach((el) => {
        cleanContent = cleanContent.replace(el.outerHTML, `[Card Block ID: ${el.getAttribute('data-card-id')}]`);
      });
      ...
      const response = await fetch('/api/posts', {
        ...
        body: JSON.stringify({
          title,
          content: outputFormat === 'markdown' ? turndownService.turndown(cleanContent) : cleanContent,
          slug,
