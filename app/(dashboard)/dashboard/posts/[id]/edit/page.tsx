  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        setMetaDescription(post.metaDescription || post.content.replace(/<[^>]+>/g, "").substring(0, 160));
        ...
        if (post.content && post.media && post.media.length > 0) {
          ...
          images.forEach((img: HTMLImageElement, index) => {
            ...
            const imgBaseUrl = img.src.includes('?') ? img.src.split('?')[0] : img.src;
            ...
            if (mediaAltMap[imgSrc] !== undefined) {
              ...
              for (const [url, alt] of Object.entries(mediaAltMap)) {
                ...
                if (imgFilename && urlFilename && imgFilename === urlFilename) {
                  dbAlt = alt;
                  altTextFound = true;
                  console.log(`Found alt text match by filename: ${imgFilename}`);
                  break;
                }
              }
            }
            ...
            if (altTextFound) {
              ...
              console.log(`No alt text match found for image ${index}: ${imgSrc}`);
            }
          });
      ...
      if (editor.current) {
        ...
        images.forEach((img: HTMLImageElement, index) => {
          ...
          if (badge) {
            ...
            console.log(`Badge updated for image ${index}: ${badge.textContent}`);
          }
        });
      }
    };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => router.push(`/dashboard/posts/27f4f873-46bd-4902-8b73-6eda8125dbd8/edit`)}>Cancel</Button>
      </div>
                ...
                const forceAltText = () => {
                  try {
                    ...
                    images.forEach((img, index) => {
                      if (img.alt) {
                        console.log(`Setting alt text on image ${index} in editor to: "${img.alt}"`);
                        ...
                        if (!found) {
                          console.log(`Could not find image node for ${img.src} - trying direct DOM update`);
                          // Direct DOM manipulation as fallback
                          setTimeout(() => {
                            const editorImgs = document.querySelectorAll('.ProseMirror img');
                            editorImgs.forEach(editorImg => {
                              if (editorImg.src === img.src) {
                                console.log(`Direct DOM update of alt text for ${img.src}`);
                                editorImg.setAttribute('alt', img.alt);
                                
                                // Also update badge if it exists
                                const container = editorImg.closest('.image-container');
                                if (container) {
                    ...
                    setTimeout(() => {
                      const editorImgs = document.querySelectorAll('.ProseMirror img');
                      console.log(`Found ${editorImgs.length} images in editor after delay`);
                      
                      // Direct badge update for all images that have alt text
                      editorImgs.forEach((editorImg: HTMLImageElement) => {
                        if (editorImg.alt && editorImg.alt.trim() !== '') {
                          console.log(`Direct badge update for image with alt: "${editorImg.alt}"`);
                          ...
                          if (container) {
                            ...
                            if (nextElement && nextElement.classList.contains('image-alt-badge')) {
                              nextElement.textContent = 'ALT';
                              nextElement.style.backgroundColor = '#4b5563';
                            }
                          }
                        }
                      });
                    ...
                    console.error("Error preserving alt text on editor init:", error);
                  }
                };
